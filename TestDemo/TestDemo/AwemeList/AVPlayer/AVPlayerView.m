...
@property (nonatomic ,strong) NSString             *sourceSheme;
...
@property (nonatomic, strong) dispatch_queue_t     cancelQueue;
@end
...
-(instancetype)initWithFrame:(CGRect)frame {
    ...
    if(self) {
        ...
        self.cancelQueue = dispatch_queue_create("com.start.cancelqueue", DISPATCH_QUEUE_CONCURRENT);
...
-(void)setPlayerWithUrl:(NSString *)url {
    ...
    self.sourceSheme = components.scheme;
    //路径作为视频缓存key
...
-(void)cancelLoading {
    ...
    dispatch_async(self.cancelQueue, ^{
...
- (BOOL)resourceLoader:(AVAssetResourceLoader *)resourceLoader shouldWaitForLoadingOfRequestedResource:(AVAssetResourceLoadingRequest *)loadingRequest {
    ...
    if(self.task == nil) {
        ...
        NSURL *URL = [[loadingRequest.request URL].absoluteString urlScheme:self.sourceSheme];
...
- (void)resourceLoader:(AVAssetResourceLoader *)resourceLoader didCancelLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest {
    ...
    NSLog(@"startOffset:%lld  numberOfBytesToRespondWidth:%lu", startOffset, (unsigned long)numberOfBytesToRespondWidth);
}
...
- (BOOL)respondWithDataForRequest:(AVAssetResourceLoadingRequest *)loadingRequest {
    ...
    unsigned long long contentLength = [self.response expectedContentLength];
    NSString *rangeValue = [self.response allHeaderFields][@"Content-Range"];
    if (rangeValue) {
    ...
    loadingRequest.contentInformationRequest.contentLength = contentLength;//[self.response expectedContentLength];
